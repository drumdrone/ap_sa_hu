  ...
  for (const section of sections) {
    ...
    for (const item of section.items) {
      ...
      const titleHtml = item.url
        ? `<a href="${escapeHtml(item.url)}" style="color:#111827;text-decoration:underline;font-weight:600;">${title}</a>${starsHtml}`
      ...
      const urlNote = item.url
        ? `<div style="font-size:12px;margin-top:3px;"><a href="${escapeHtml(item.url)}" style="color:#6b7280;text-decoration:underline;">${escapeHtml(shortenUrl(item.url))}</a></div>`
        : "";
